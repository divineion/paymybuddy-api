...
public class TransferService {
	...
	public TransferService(TransferMapper mapper, TransferRepository repository, UserService userService, UserMapper userMapper) {
		this.mapper = mapper;
	...
	public TransferDto createTransfer(TransferRequestDto transferReqDto) throws UserNotFoundException, InsufficientBalanceException {
		// FETCH SENDER infos
		UserDto senderDto = userService.findUserById(transferReqDto.senderId());
		// business validation
		if (transferReqDto.amount().compareTo(senderDto.balance()) == 1) {
		...
		BigDecimal updatedSenderBalance = sender.getBalance().subtract(amount.add(amount.multiply(ServiceFees.TRANSFER_FEES)));
